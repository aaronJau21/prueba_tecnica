...
@section('contenido')
    ...
    <form action="{{ route('update_task', $task->id) }}" method="POST"
        ...
        @csrf
        @method('PUT')
        <h2 class="text-2xl font-semibold text-gray-800 mb-6 text-center">Formulario de Editar</h2>
        ...
        <div class="mb-4">
            ...
            <input type="text" id="titulo" name="titulo"
                ...
                placeholder="Ingresa Título" value="{{ $task->titulo }}">
            @error('titulo')
        ...
        <div class="mb-4">
            ...
            <textarea id="descripcion" name="descripcion"
                class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder="Ingresa Descripción">{{ $task->descripcion }}</textarea>
            @error('descripcion')
                <p class="text-red-700">{{ $message }}</p>
            @enderror
        </div>
        ...
        <!-- status -->
        <div class="mb-4">
            <label for="user_id" class="block text-sm font-medium text-gray-700">Estado del proyecto</label>
            {{-- {{ dd($users) }} --}}
            <select name="estado" id="estado"
                class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                <option value="">Seleccione un Estado</option>
                <option value="pendiente" {{ old('estado', $task->estado) === 'pendiente' ? 'selected' : '' }}>
                    Pendiente
                </option>
                <option value="progreso" {{ old('estado', $task->estado) === 'progreso' ? 'selected' : '' }}>
                    Progreso
                </option>
                <option value="completado" {{ old('estado', $task->estado) === 'completado' ? 'selected' : '' }}>
                    Completado
                </option>
